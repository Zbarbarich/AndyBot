  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <div className="relative">
        <Search className={`absolute left-3 top-1/2 -translate-y-1/2 text-text-muted pointer-events-none ${compact ? 'w-4 h-4' : 'w-4 h-4'}`} />
        <input
          ref={inputRef}
          type="search"
          value={query}
      ...
      {open && query.trim() && (
        <div
          className="absolute left-0 right-0 top-full mt-1 z-50 glass-panel shadow-xl max-h-[70vh] overflow-y-auto sm:max-w-none"
          role="listbox"
        >
          {loading ? (
            ...
            <div className="py-2">
              {results!.customers.length > 0 && (
                <div className="px-3 py-1">
                  <div className="flex items-center gap-2 text-xs font-medium text-text-muted uppercase tracking-wide mb-1">
                  ...
                  {results!.customers.map((c) => (
                    <Link
                      ...
                      className="block px-3 py-2 rounded hover:bg-surface-elevated text-text text-sm"
                      role="option"
                    >
                      {c.name}
                    </Link>
                ...
                <div className="px-3 py-1">
                  ...
                  {results!.purchase_orders.map((po) => (
                    <Link
                      key={po.id}
                      to={`/purchasing/${po.id}`}
                      onClick={() => handleLinkClick(`/purchasing/${po.id}`)}
                      className="block px-3 py-2 rounded hover:bg-surface-elevated text-text text-sm"
                      role="option"
                    >
