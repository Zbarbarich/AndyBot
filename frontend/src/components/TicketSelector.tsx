  ...
  return (
    <div ref={wrapperRef} className={`relative ${className}`}>
      <div className="mb-1.5">
        <label className="block text-sm font-medium text-dark-text-muted mb-1">Ticket</label>
        <input
          type="text"
          value={searchQuery}
      ...
      {showSearchResults && (searchQuery.trim() || searchResults.length > 0) && (
        <div className="absolute z-20 mt-0 w-full max-h-48 overflow-auto rounded-lg border border-dark-border bg-dark-surface shadow-lg">
          {searching ? (
            <div className="p-2 text-dark-text-muted text-sm">Searching…</div>
              ...
              {searchResults.map((t) => (
                <li key={t.id}>
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-sm text-dark-text hover:bg-dark-surface-elevated"
                  >
                    #{t.id} {t.subject}
                  </button>
                </li>
            ...
            <div className="p-2 text-dark-text-muted text-sm">No tickets found.</div>
          )}
        </div>
      )}
      <div>
        <label className="block text-sm font-medium text-dark-text-muted mb-1 sr-only">Selected ticket</label>
        <select
          value={value || ''}
