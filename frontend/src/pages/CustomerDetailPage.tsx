...
const pillTabClass = (active: boolean) =>
  `px-3 py-2 font-medium rounded-lg transition-colors min-h-[44px] whitespace-nowrap ${
    active ? 'bg-surface-elevated text-primary' : 'text-text-muted hover:text-text'
  }`;
...
const CustomerDetailPage = () => {
  ...
  const invoices = invoicesTab === 'open' ? openInvoices : closedInvoices;
    ...
    <div className="page-container">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="lg:col-span-1">
          <div className="rounded-xl bg-surface border border-border p-4 sm:p-6 lg:sticky lg:top-4">
            <h2 className="text-lg font-semibold text-text mb-3">Customer</h2>
            <dl className="space-y-2 text-sm text-text">
              ...
              {customer.contact_name && (
                <div>
                  <dt className="text-text-muted text-xs">Contact (POC)</dt>
              ...
              <div>
                <dt className="text-text-muted text-xs">Text notifications</dt>
                <dd>{customer.text_notifications ? 'Yes' : 'No'}</dd>
              </div>
              <div>
                <dt className="text-text-muted text-xs">Email notifications</dt>
                <dd>{customer.email_notifications ? 'Yes' : 'No'}</dd>
              </div>
              <div>
                <dt className="text-text-muted text-xs">Ticket #s</dt>
                <dd>{customer.ticket_ids?.length ? customer.ticket_ids.join(', ') : 'None'}</dd>
              </div>
            </dl>
            <div className="mt-4 pt-4 border-t border-border flex flex-wrap gap-2">
              <button type="button" onClick={() => navigate(`/customers/${customer.id}/payment-history`)} className="btn-secondary text-sm py-1.5 px-3 min-h-[44px]">Payment history</button>
              <button type="button" onClick={openEditModal} className="btn-secondary text-sm py-1.5 px-3 min-h-[44px]">Edit</button>
              <button type="button" onClick={handleDelete} className="btn-secondary text-sm text-red-400 py-1.5 px-3 min-h-[44px]">Delete</button>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <section>
              ...
              <div className="flex gap-2 mb-3 overflow-x-auto">
                <button type="button" onClick={() => setActiveTab('open')} className={pillTabClass(activeTab === 'open')}>
                  Open ({openTickets.length})
                </button>
                <button type="button" onClick={() => setActiveTab('closed')} className={pillTabClass(activeTab === 'closed')}>
              ...
              <ResponsiveEntityList
                items={tickets}
                getKey={(t) => t.id}
                emptyMessage={activeTab === 'open' ? 'No open or pending tickets.' : 'No closed tickets.'}
                renderCard={(t) => (
                  <ListCardRow
            ...
            <section>
              <h2 className="text-lg font-semibold text-text mb-3">Orders</h2>
              <div className="flex gap-2 mb-3 overflow-x-auto">
                <button type="button" onClick={() => setOrdersTab('open')} className={pillTabClass(ordersTab === 'open')}>
                  Open ({openOrders.length})
                </button>
                <button type="button" onClick={() => setOrdersTab('closed')} className={pillTabClass(ordersTab === 'closed')}>
                  ...
                  <ListCardRow
                    ...
                    meta={
                      <>
                        ...
                        <span>{formatDate(o.order_date ?? o.valid_until ?? null)}</span>
                      </>
                    }
            ...
            <section>
              <h2 className="text-lg font-semibold text-text mb-3">Invoices</h2>
              <div className="flex gap-2 mb-3 overflow-x-auto">
                <button type="button" onClick={() => setInvoicesTab('open')} className={pillTabClass(invoicesTab === 'open')}>
                  Open ({openInvoices.length})
                </button>
                <button type="button" onClick={() => setInvoicesTab('closed')} className={pillTabClass(invoicesTab === 'closed')}>
              ...
              <ResponsiveEntityList
                ...
                renderCard={(inv) => (
                  <ListCardRow
                    ...
                    meta={
                      <>
                        <span>Total ${Number(inv.total).toFixed(2)}</span>
                        <span>Paid ${Number(inv.amount_paid ?? 0).toFixed(2)}</span>
                        <span>Due ${Number(inv.balance_due ?? inv.total - (inv.amount_paid ?? 0)).toFixed(2)}</span>
                      </>
                    }
      ...
      {showEditModal && (
        <div className="modal-overlay safe-area-pb" role="dialog" aria-modal="true" aria-labelledby="edit-customer-title">
          <div className="modal-content">
            <h2 id="edit-customer-title" className="text-lg font-semibold text-text mb-4">Edit Customer</h2>
            <form onSubmit={handleEditSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-text-muted mb-1">Contact name (POC)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-text-muted mb-1">Physical address</label>
                <input
                  type="text"
