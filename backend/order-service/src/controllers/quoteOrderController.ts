...
async function enrichOrderLinesForDetail(
  ...
  const invMap = new Map<number, Array<{ invoice_id: number; sub_order_number: string; invoice_number: string; quantity: number }>>();
...
export const quoteOrderController = {
  ...
  async cancelQuote(req: AppRequest, res: Response): Promise<void> {
    try {
      const id = parseInt(req.params.id, 10);
      if (isNaN(id)) {
        res.status(400).json({ error: 'Invalid id' });
      ...
      if (doc.status === 'converted') {
        res.status(400).json({ error: 'Cannot cancel a converted quote' });
        return;
      }
      if (doc.status === 'closed') {
        res.status(400).json({ error: 'Quote is already closed' });
        return;
      }
      const result = await query(quoteOrderQueries.setQuoteStatusClosed, [id]);
      if (result.rows.length === 0) {
        res.status(400).json({ error: 'Quote could not be cancelled' });
        return;
      }
      const withCustomer = await query(quoteOrderQueries.getByIdWithCustomer, [id]);
      const linesResult = await query(quoteOrderQueries.getLinesByQuoteOrderId, [id]);
      res.json({ ...withCustomer.rows[0], lines: linesResult.rows });
    } catch (e) {
      console.error('quoteOrderController.cancelQuote', e);
