...
Migrations run in order (000–022). This doc summarizes tables and foreign keys for reference.
...
- **users.ui_preferences** (`022`) — JSONB; per-user UI settings (e.g. `tableColumns`, `sidebarCollapsed`). Not secrets.
- **purchase_order_lines.ordered_at** — null means unpurchased; used for dashboard “open unpurchased items” counts.
- **ticket_resolution_updates** — progress notes; ticket list/dashboard treats zero notes or last activity older than 14 days as stale.
...
All connections should set `timezone = 'America/New_York'` so `NOW()` and `CURRENT_DATE` are Eastern. Application and PDF layer format calendar dates as **MM-DD-YY** in America/New_York (`formatDate` / `formatDatePdf`). Exact instants remain in the database as timestamps.
